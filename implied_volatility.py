...
def main():
    ...
    n = len(dates)
    m = numpy.empty(n, dtype=numpy.int32)
    y = numpy.empty(n, dtype=numpy.double)
    xmin = numpy.empty(n, dtype=numpy.double)
    xmax = numpy.empty(n, dtype=numpy.double)

    # Need to sort for NAG Algorithm
    data = data.sort('Strike')
	...
	call_data = data[(data.Expiration == plot_date) &
        ...
        if exp_sizes > 0:
            ...
            if (i == 0):
                ...
                x = x.append(call_data.Strike)
                call = call.append(call_data.impvolPut)
                xmin[i] = x.min()
                xmax[i] = x.max()
            i+=1
